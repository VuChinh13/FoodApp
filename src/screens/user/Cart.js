...
const Cart = ({ navigation }) => {
  ...
  const addItem = async item => {
    ...
    let tempDart = user._data.cart.map(itm => {
      if (itm.id === item.id) {
        return {
          ...itm,
          qty: Number(itm.qty) + 1,  // ✔ qty gán thẳng vào item
        };
      }
      return itm;
    });
  ...
  const removeItem = async item => {
    ...
    let tempDart = user._data.cart
      .map(itm => {
        if (itm.id === item.id) {
          const updatedQty = Number(itm.qty) - 1;
          if (updatedQty <= 0) return null; // xoá item khỏi cart
          return {
            ...itm,
            qty: updatedQty,   // ✔ không gán vào data
          };
        }
        return itm;
      })
